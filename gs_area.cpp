...
GSSRIDUnitEnum C_WRK_SESSION::get_SRID_unit(void)
{
   if (SRID_unit == GSSRIDUnit_None)
      // Se è stato dichiarato un sistema di coordinate per la sessione corrente
...
int C_WRK_SESSION::thaw_1part(bool RecoverDwg)
{
   ...
   do
   {
      ...
      // chiamo il comando open (che verrà ridefinito da GEOsim) e non il nativo .open perchè una volta scongelato
                           ...
                           C_SELSET *pExtractedSS, int retest)
{
   ...
   if (nClsToExtract > 1) // se si devono estrarre più di una classi
   {
   ...
   do
   {
             ...
             ++times <= GEOsimAppl::GLOBALVARS.get_NumTest())
      {
         ...
         if (nClsToExtract > 1) // se si dovevano estrarre più di una classi
         {
...
/*+
  Questa funzione è la versione funzione LISP della "gsc_getLastExtractedClassesFromINI".
  Parametri:
